...
static void _processQXferCommand(struct GDBStub* stub, const char* params, const char* data) {
	...
	++index;
	for (; index + 3 < strlen(params); ++index) {
		length <<= 4;
		length |= _hex2int(&params[index], 1);
	}

	if (length + 1 > GDB_STUB_MAX_LINE) {
    length = GDB_STUB_MAX_LINE - 1;
	}

	memset(stub->outgoing, 0, GDB_STUB_MAX_LINE);
	...
	strncpy(&stub->outgoing[1], &data[offset], length);
}
...
static void _generateMemoryMapXml(struct GDBStub* stub, char* memoryMap) {
	...
	for (i = 0; i < nBlocks; ++i) {
		...
		char* type = blocks[i].flags & mCORE_MEMORY_WRITE ? "ram" : "rom";
	...
	strncpy(&memoryMap[index], "</memory-map>", 14);
}
